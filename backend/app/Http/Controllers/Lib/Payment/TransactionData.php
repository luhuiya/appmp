...
class TransactionData extends Payment
{
    ...
    */
	...
	protected function _getNextStatementId()
	{
		
		$next_id = \DB::table('trans_payment')->max('id');
		
		return intval($next_id)+1;
	...
	protected function _createPaypalPaymentPaypal(){
		...
		//s		et the trasaction id , make sure $_paymentId var is set within your class
		$this->_paymentId = $createPayment->id;
		
		
		//d		ump the repose data when create the payment
	...
	public function _createPayment(){
		...
		if($this->payment_method == '01') {
			...
			$response = $this->_balanceUpdate();
			
		}
		else {
			
			$response = $this->_savePayment();
	...
	private function _savePayment() {
		...
		$next_id = $this->_getNextStatementId();
		...
		$trans_payment->token = $token_download;
